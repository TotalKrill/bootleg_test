...
pub const MESSAGE_CHANNEL_ID: MessageChannelID = MessageChannelID::new(0);
...
pub fn setup(mut commands: Commands, tokio_rt: Res<Runtime>, task_pool: Res<IoTaskPool>) {
...
pub fn send(mut net: ResMut<NetworkResource>, time: Res<wasm_timer::SystemTime>) {
    let message = format!("Hello from client! {:?}", *time);
    net.broadcast_message(&message, &MESSAGE_CHANNEL_ID);
}

pub fn receive(mut net: ResMut<NetworkResource>) {
